...
SubShader {
    ...
    Pass
    {
        ...
        struct pixel_t
        {
            ...
            float4	color           : COLOR;
            float4	atlas           : TEXCOORD0;    // xy = UV, z = gradientScale, w = xScaleVal
            float2	param           : TEXCOORD1;    // x = baseWeight, y = spreadRatio
            float2	mask            : TEXCOORD2;    // Position in object space(xy)
            float3	viewDir         : TEXCOORD3;

            #if (UNDERLAY_ON || UNDERLAY_INNER)
            float2	texcoord2       : TEXCOORD4;    // underlayUV
        ...
        pixel_t VertShader(vertex_t input)
        {
            ...
            output.color = color;
            output.atlas = float4(input.texcoord0.xy, input.texcoord0.z, abs(input.texcoord0.w));
            output.param = float2(baseWeight, spreadRatio);
            output.mask = half2(vert.xy * 2 - clampedRect.xy - clampedRect.zw);
            output.viewDir = mul((float3x3)_EnvMatrix, _WorldSpaceCameraPos.xyz - mul(unity_ObjectToWorld, vert).xyz);
            #if (UNDERLAY_ON || UNDERLAY_INNER)
            output.texcoord2 = input.texcoord0.xy + layerOffset;
        ...
        fixed4 PixShader(pixel_t input) : SV_Target
        {
            ...
            float spreadRatio = input.param.y;
            ...
            pixelSize *= _MainTex_TexelSize.w * 0.75;
            float baseScale = 1 / pixelSize * (_Sharpness + 1);

            float xScaleVal = input.atlas.w;
            float gradientScale = input.atlas.z;
            ...
            result = input.underlayColor * (1 - saturate(ud - layerBias)) * faceMask;
            #endif
            ...
            float3 n = GetSurfaceNormal(input.atlas.xy, baseWeight, dxy, gradientScale);
            ...
            float sdGlow = (0.5 - normalizedFaceEffect + 0.5 / scale - tex2D(_MainTex, input.atlas.xy).a) * scale;
